-- Subscription and Billing Database Schema (Clean Version)
-- This file contains all the tables and functions needed for the commercialization service
-- NO user_id references - compatible with existing database structure
...
-- 8. Create RLS Policies (Completely simplified - no user_id references)
...
-- Subscriptions are readable by all authenticated users (simplified)
CREATE POLICY "subscriptions_read_policy" ON subscriptions
    FOR SELECT USING (true);

-- Subscriptions can be created/updated by all authenticated users (simplified)
CREATE POLICY "subscriptions_write_policy" ON subscriptions
    FOR ALL USING (auth.role() = 'authenticated');

-- Billing history is readable by all authenticated users (simplified)
CREATE POLICY "billing_history_read_policy" ON billing_history
    FOR SELECT USING (true);

-- Usage tracking is readable by all authenticated users (simplified)
CREATE POLICY "usage_tracking_read_policy" ON usage_tracking
    FOR SELECT USING (true);

-- Usage tracking can be updated by all authenticated users (simplified)
