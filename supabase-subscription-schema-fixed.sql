-- Subscription and Billing Database Schema (Fixed Version)
...
-- 8. Create RLS Policies (Simplified for existing structure)
...
-- Subscriptions are readable by organization members (simplified)
CREATE POLICY "subscriptions_read_policy" ON subscriptions
    FOR SELECT USING (true);

-- Subscriptions can be created/updated by authenticated users (simplified)
CREATE POLICY "subscriptions_write_policy" ON subscriptions
    FOR ALL USING (auth.role() = 'authenticated');

-- Billing history is readable by authenticated users (simplified)
CREATE POLICY "billing_history_read_policy" ON billing_history
    FOR SELECT USING (auth.role() = 'authenticated');

-- Usage tracking is readable by authenticated users (simplified)
